...
const QR_MAIN='https://cdnjs.cloudflare.com/ajax/libs/qr-scanner/1.0.0/qr-scanner.umd.min.js';
const QR_WORKER='https://cdnjs.cloudflare.com/ajax/libs/qr-scanner/1.0.0/qr-scanner-worker.min.js';
...
window.EVIA_NAXOS={
  ...
  async scanCourse(){
    if(!window.QrScanner)await loadScript(QR_MAIN);
    ...
    return new Promise((resolve,reject)=>{
      ...
      try{if(window.QrScanner.WORKER_PATH)window.QrScanner.WORKER_PATH=QR_WORKER;scanner=new window.QrScanner(video,onDecode,{returnDetailedScanResult:true,highlightScanRegion:true,highlightCodeOutline:true,onDecodeError:()=>{}});scanner.start().then(()=>status.textContent='Camera ready — scan the QR code.').catch(e=>finish(e))}catch(e){finish(e)}
    });
  }
};
...
function injectPackage(url){return new Promise((resolve,reject)=>{const done=e=>{window.removeEventListener('NAXOS4_COURSE_PACKAGE_READY',ready);window.removeEventListener('NAXOS4_COURSE_PACKAGE_ERROR',error);e.type==='ok'?resolve(e.detail):reject(new Error(e.detail?.message||'Course package could not be loaded.'))};const ready=e=>done({type:'ok',detail:e.detail});const error=e=>done({type:'error',detail:e.detail});window.addEventListener('NAXOS4_COURSE_PACKAGE_READY',ready);window.addEventListener('NAXOS4_COURSE_PACKAGE_ERROR',error);const s=document.createElement('script');s.src=url;s.onload=()=>{};s.onerror=()=>done({type:'error',detail:{message:'The Naxos course package could not be reached.'}});document.head.appendChild(s)})}
